...
func _physics_process(delta):
	if is_instance_valid(player):
		match current_state:
			...
			state.ATTACKING:
				move_and_attack()
				#$mutant/AnimationPlayer.play("Standing Melee Combo Attack Ver 3-loop")
			state.RETURNING:
				move_and_attack()
				#$mutant/AnimationPlayer.play("Standing Melee Attack 360 High-loop")
			state.RESTING:
				#$mutant/AnimationPlayer.play("Happy Idle-loop")
				pass
	#			print("Resting!")			
...
#	print("I'm this far away from my target: ", distance)
	...
	if distance < 100:
		...
		var cnt=randi() % 100
		if cnt%2==0:
			$mutant/AnimationPlayer.play("Standing Melee Combo Attack Ver 1-loop")
			cnt+=1
			cnt%=2
		else:
			$mutant/AnimationPlayer.play("Standing Melee Combo Attack Ver 3-loop")
